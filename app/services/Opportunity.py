...
def _profile_string_list(value) -> list[str]:
    """Normalize profile field (str, list[str], or list[{name}]) to distinct non-empty strings."""
    if value is None:
        return []
    if not isinstance(value, list):
        s = OpportunityTextBuilder._item_text(value)
        return [s] if s else []
    seen: set[str] = set()
    out: list[str] = []
    for item in value:
        s = OpportunityTextBuilder._item_text(item)
...
class OpportunityService:
    ...
    async def get_matched_opportunities_for_speaker(
        ...
        topics = _profile_string_list(profile.get("topics"))
    ...
    async def run_matching_and_save(
        ...
        **_kwargs,
        ...
        """
        Match opportunities via Mongo field filters and save ids to matchedOpportunities.
        When matched_entry_id is provided (from match-by-speaker flow), updates that entry
        to status 'completed'.
        ...
        opportunity_ids = [str(o.get("_id")) for o in opportunities if o.get("_id") is not None]
        await _finish(opportunity_ids)

        if opportunities and await self._should_send_matched_email_after_match(
            profile,
            ...
            try:
                ...
                await mailer.send_matched_opportunities_email(
                    speaker_profile_id,
                    opportunity_documents=opportunities,
                )
