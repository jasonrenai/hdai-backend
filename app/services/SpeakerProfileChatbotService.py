...
class SpeakerProfileChatbotService:
    ...
    async def process_chat(
        ...
        """
        Conversational onboarding via SpeakerOnboardingAgent.

        Backend owns workflow (next question, validation, completion).
        LLM only analyzes language (intent + extraction).
        """
        from app.agents.SpeakerOnboardingAgent import SpeakerOnboardingAgent

        agent = SpeakerOnboardingAgent(self)
